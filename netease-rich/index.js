...
const load = async () => {
  const { token, cookiesStr } = await login();
  ...
  const xls = json2xls(formatData(resultArr));
  fs.writeFileSync('netease-rich-format.xlsx', xls, 'binary');
}
...
https://qian.163.com/pc/xhr/data/bill/fund/list.do?&token=HvqmlTVwyt4wNCHk&__searchtime=1604305929251
